...
void print_individual_metrics(const char *algo, Process procs[], int n) {
    printf("---- %s Individual Metrics ----\n", algo);
    for (int i = 0; i < n; ++i) {
        ...
        printf("P%d: Turnaround=%d, Waiting=%d, Response=%d\n",
               procs[i].pid, turnaround, waiting, response);
    }
    printf("\n");
}
...
void run_test_case(const char *testName, Process procs[], int n, int quantum,
    ...
    Process fcfsCopy[n];
    memcpy(fcfsCopy, procs, sizeof(Process) * n);
    fcfs_metrics(fcfsCopy, n);
    print_individual_metrics("FCFS", fcfsCopy, n);
    ...
    Process sjfCopy[n];
    memcpy(sjfCopy, procs, sizeof(Process) * n);
    sjf_metrics(sjfCopy, n);
    print_individual_metrics("SJF", sjfCopy, n);
    ...
    Process rrCopy[n];
    memcpy(rrCopy, procs, sizeof(Process) * n);
    rr_metrics(rrCopy, n, quantum);
    print_individual_metrics("RR", rrCopy, n);
